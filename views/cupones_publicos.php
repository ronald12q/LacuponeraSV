...
<body class="bg-[#1D212D] min-h-screen text-white font-sans">
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="max-w-2xl mx-auto mb-10">
            <form action="?url=home/buscar" method="GET" class="relative">
                <input type="hidden" name="url" value="home/buscar">
        ...
        <div class="flex flex-wrap justify-center gap-3 mb-8">
            <a href="?url=home/cupones" class="px-4 py-2 rounded-full text-sm <?php echo !isset($categoriaActual) ? 'bg-[#7132d1] text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'; ?> transition-colors">
                Todos
            </a>
            <?php foreach($categorias as $cat): ?>
                <a href="?url=home/categoria&cat=<?php echo urlencode($cat); ?>" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php foreach($cupones as $cupon): ?>
                    <?php 
                        ...
                        $colorIndex = crc32($cupon['Categoria'] ?? 'default') % count($colores);
                        $colorClase = $colores[$colorIndex];
                        
                      
                        $descuento = 0;
                        if($cupon['precio_regular'] > 0) {
                            $descuento = round((($cupon['precio_regular'] - $cupon['precio_oferta']) / $cupon['precio_regular']) * 100);
                        }
                    ?>
                    <div class="bg-[#1E233A] rounded-2xl overflow-hidden shadow-xl transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                        ...
                        <div class="p-6">
                            ...
                            <h3 class="text-[#7c7cff] text-lg font-medium mb-3">
                                <?php echo htmlspecialchars($cupon['titulo']); ?>
                            </h3>
                            
                            
                            <div class="flex justify-between items-center text-sm text-gray-400 mb-4">
                                <span><?php echo htmlspecialchars($cupon['nombre_empresa']); ?></span>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <a href="?url=home/verCupon&id=<?php echo $cupon['id_oferta']; ?>" 
